...
function ftpDown(mulu1, mulu2)
    if version ~= "2.2.4" and whoAmI() == 1 then
        ...
        _, err = ftp.download(mulu1, mulu2)
...
function ftpUp(fileName, muLu)
    if version ~= "2.2.4" and whoAmI() == 1 then
        ...
        if muLu == nil then
            _, err = ftp.upload(luaMuLu .. fileName, ftpMuLu)
        else
            _, err = ftp.upload(luaMuLu .. fileName, ftpMuLu .. muLu)
        end
        if err then
            ftpUpNo = ftpUpNo + 1
            if ftpUpNo == 5 then
...
function ftpUpPro(csMulu, csFuWuQi)
    if version ~= "2.2.4" and whoAmI() == 1 then
        ...
        _, err = ftp.upload(csMulu, csFuWuQi)
        if err then
            ftpUpNo = ftpUpNo + 1
            if ftpUpNo == 5 then
                ftpUpNo = 0
...
function ftpUpPNG(fileName, muLu)
    if version ~= "2.2.4" and whoAmI() == 1 then
        ...
        _, err = ftp.upload(luaPath .. "/res/" .. fileName, ftpMuLu .. muLu)
        if err then
            ftpUpNo = ftpUpNo + 1
            if ftpUpNo == 5 then
                ftpUpNo = 0
...
function ftpDel(fileName)
    if version ~= "2.2.4" and whoAmI() == 1 then
        ...
        _, err = ftp.cmd("ftp://xinqinew:Qwer1234@192.168.0.251/video/qj/", "DELE 1.lua")
    end
end
...
function ftpList(muLu)
    if version ~= "2.2.4" and whoAmI() == 1 then
        ...
        msg, err = ftp.cmd(ftpMuLu .. muLu, "LIST")
        return msg
    end
end
        ...
        if tmpNum ~= nil and tmpNumBenDi ~= nil then
            if tmpNum ~= tmpNumBenDi then
                ...
                if tmpShou - tmpWei <= 5 then
                    ...
                    for i = 1, 10 do
                        _, err = ftp.upload(luaPath .. "/lua/shou.plist", ftpMuLu .. "xinxi/")
                        if err then
                        else
                            break
                        end
                    ...
                    for i = 1, 10 do
                        _, err = ftp.upload(luaPath .. "/lua/wei.plist", ftpMuLu .. "xinxi/")
                        if err then
                        else
                            break
                        end
        ...
        if tmpNum ~= nil and tmpNumBenDi ~= nil then
            if tmpNum ~= tmpNumBenDi then
                ...
                if tmpZhi <= 5 and tmpZhi > 0 then
                    ...
                    for i = 1, 10 do
                        _, err = ftp.upload(luaPath .. "/lua/now.plist", ftpMuLu .. "xinxi/")
                        if err then
                        else
                            break
                        end
    ...
    if tmpXuanXiang ~= nil then
        if isFileJiZhongJianCe == false then
            if nowDateTime.min >= 50 and nowDateTime.min <= 60 then
                ...
                for i = 1, 10 do
                    _, err = ftp.upload("/User/Media/TouchSprite/res/xinxi.txt", ftpMuLu .. "xinxi/")
                    if err then
                    else
                        break
                    end
...
function chuCiJiaZai(...)
    ...
    -- end
    -- 日期早
    nowDay = loadPlist(luaMuLu .. xiangMu .. ".plist", "日期")
    if nowDay == nil then
        nowDay = nowDateTime.day
        writePlist(luaMuLu .. xiangMu .. ".plist", "日期", nowDay)
    end
    -- 日期晚
    nowDayNight = loadPlist(luaMuLu .. xiangMu .. ".plist", "日期晚")
    ...
    -- end
    numDay = loadPlist(luaMuLu .. xiangMu .. ".plist", "第几天")
    if numDay == nil then
        numDay = 0
...
function jiaoBenGuZhangJianCe(...)
    if isJiaoBenJianCe == false then
        ...
        if nowDateTime.min >= tmpMin and nowDateTime.min <= 45 then
            ...
            for i = 1, 10 do
                _, err = ftp.upload(luaPath .. "/res/" .. iphoneId .. ".txt", ftpMuLu .. xiangMu .. "/jb/")
                if err then
                else
                    break
                end
            ...
            for i = 1, 10 do
                _, err = ftp.upload(luaPath .. "/res/" .. iphoneId .. "jb1.jpg", ftpMuLu .. xiangMu .. "/RMB/")
                if err then
                else
                    break
                end
...
function loadUpXinXi(...)
    ...
    for i = 1, 10 do
        _, err = ftp.upload("/User/Media/TouchSprite/res/" .. iphoneId .. ".txt", ftpMuLu .. "xinxi/")
        if err then
        else
            break
        end
...
function onlineLua()
    ...
    local temRet = ftpDown(ftpMuLu .. "Lua/riseOfStars.lua", userPath() .. "/lua/riseOfStars.lua")
    if temRet then
        toast("FTP下载成功", 1)
        mSleep(1000)
    else
